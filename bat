...
local Hats = {
    xd = Character:WaitForChild("Jackette's SluggerAccessory"), -- the hat/body limb name

}
Hats.xd.Handle.AccessoryWeld:Destroy()
Hats.xd.Handle.BodyBackAttachment:Destroy()
local function align(i,v) -- puts everything in place
    att0 = Instance.new("Attachment", i)
    att0.Position = Vector3.new(0,0,0) 
    att1 = Instance.new("Attachment", v)
    att1.Position = Vector3.new(-0.1,-1,-1) --0.5,-1,0
    att1.Rotation = Vector3.new(-55, 90, 180) -- -90, 90, 180
	...
	if not using then
		...
		local function dmg(part)
			...
			if hum ~= nil then
				...
				baseballhit:Play()
				baseballhit1:Play()
				hum.Health = hum.Health - math.random(0,0)
